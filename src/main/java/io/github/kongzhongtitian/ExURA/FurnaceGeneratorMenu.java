...
public class FurnaceGeneratorMenu extends AbstractContainerMenu {
    ...
    public FurnaceGeneratorMenu(int containerId, Inventory inv, FriendlyByteBuf extraData) {
        super(ExURAMenu.FURNACE_GENERATOR_MENU.get(), containerId);
        BlockPos pos = extraData.readBlockPos();
        BlockEntity entity = inv.player.level().getBlockEntity(pos);
        if (!(entity instanceof FurnaceGeneratorBlockEntity be)) {
    ...
    public FurnaceGeneratorMenu(int containerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(ExURAMenu.FURNACE_GENERATOR_MENU.get(), containerId);
        checkContainerSize(inv, 1);

        this.blockEntity = (FurnaceGeneratorBlockEntity) entity;
        this.level = inv.player.level();
        this.data = data;
        ...
        blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER).ifPresent(handler -> {
            ...
            this.addSlot(new SlotItemHandler(handler, 0, 80, 35) {
                ...
                public boolean mayPlace(ItemStack stack) {
                    return FurnaceGeneratorBlockEntity.isFuel(stack);
                }
            });
        });
    }
